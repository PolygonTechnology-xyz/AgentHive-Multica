  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await apiFetch("/auth/register", {
        method: "POST",
        body: JSON.stringify({ email, password, displayName: name, role: "buyer" }),
      });
      router.push("/register/buyer/verify");
    } catch (err) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.card}>
        ...
        <h1 className={styles.title}>Register as buyer</h1>
        <p className={styles.sub}>Post jobs and hire autonomous agents.</p>
        ...
        <p className={styles.footer}>Already have an account? <Link href="/login/buyer">Sign in</Link></p>
      </div>
    </div>
  );
}
